...
| Baseline model-profile continuity surfaces (baseline slice of `model_registry` for `reflex` / `deliberation` / `reflection`, `ticks.selected_model_profile_id`, `agent_state.current_model_profile_id`) | Baseline model-routing seam (`F-0008`) via the `F-0003` continuity transaction | Runtime and future decision harness request selection; platform health surface reads baseline diagnostics; boot/recovery reads the active profile pointer during restart/reclaim | Boot/preflight changing profile selection, `SubjectStateStore`, executive/governor/reporting workers writing profile choices directly | Baseline profile registration is router-owned; active profile pointers become durable only when committed through the runtime continuity boundary. |
| Expanded model-registry source surfaces (non-baseline slice of `model_registry`, `model_profile_health`, `model_fallback_links`) | Expanded model ecology seam (`F-0014`) | `F-0013` bounded operator projection, `CF-015` derived reporting, future decision harness/router read the richer source state; workshop/governor may later propose changes only through their own owner gates | Boot/runtime helpers mutating richer profiles directly, reporting writing back source state, Homeostat reading raw source state as a substitute for reports, workshop/specialist flows bypassing owner gates | One physical registry family may host both baseline and richer rows, but logical write authority is split by role class and explicit companion source surfaces. `F-0014` owns non-baseline rows plus richer runtime health/fallback state; it does not own reporting projections. |
...
| Homeostat snapshots and typed reaction requests (`homeostat_snapshots`, `homeostat.reaction-request`) | Homeostat seam (`F-0012`) | `F-0003` completed-tick path and the dedicated `homeostat.periodic-evaluation` scheduler family may trigger evaluation; reporting, governor and audit consumers read snapshots or consume typed requests through owner gates | Reporting/governor/runtime helpers mutating snapshots directly, arbitrary workers publishing control requests outside the typed contract | Snapshot rows are derived from committed canonical state only. Reaction requests are durable and advisory until a canonical consumer decides enforcement. |
| Workshop source surfaces (`datasets`, `training_runs`, `eval_runs`, `model_candidates`, `candidate_stage_events`) | Workshop seam (`F-0015`) | `F-0014` registry owners, future governor gates, observability and specialist lifecycle seams read workshop evidence or consume bounded packages through canonical gates | Boot/runtime/router/operator helpers writing workshop rows directly, workshop mutating active profile pointers or richer registry rows directly | Workshop owns dataset/training/eval lineage and full candidate lifecycle semantics (`candidate -> shadow -> limited-active -> active -> stable -> rollback`). Approval, live activation and specialist-specific rollout policy still remain with neighbouring owners; `promotion-package` is a bounded projection over workshop lifecycle state, not a second writer-owned store. |
| Development/governance proposal surfaces (`development_ledger`, governor-side proposal records, proposal decisions, proposal execution outcomes, freeze state) | Future governor seam (`CF-016`) | Runtime, recovery, workshop, human override and later code/body evolution flows may submit evidence, proposal requests or bounded execution-result evidence through governor-owned gates | Boot/runtime/router/subject-state seams writing arbitrary proposal rows directly; code/body evolution writing governor rows outside governor-owned gates | Proposal, decision, execution-outcome, freeze and ledger writes must flow through governor policy gates and preserve evidence plus rollback links. Workshop may submit candidate evidence or promotion packages, and `CF-012` may later consume approval/rollback evidence for execution, but neither seam owns the governor source surfaces themselves; execution outcomes are governor-side audit facts, not downstream execution authority. |
...
| Read-only reporting surfaces (identity continuity reports, model health reports, stable-snapshot inventories) | Derived observability/reporting seam (`CF-015`) | All canonical writers above provide source state; homeostat and human audit consume reports | Reporting workers back-writing identity-bearing source tables | Reports are materialized from committed source state and may not mutate the business or identity-bearing surfaces they summarize. Model/organ health reports are derived from `F-0008` baseline diagnostics plus `F-0014` richer source surfaces and remain the canonical read source for `organ_error_rate`. |
| Support/operability evidence surfaces (`support_incidents`, `support_evidence_refs`, `support_action_records`, `support_runbook_versions`) | Support and incident-discipline seam (`F-0028`) | Operators consume support runbooks and support incidents through the existing `F-0013` namespace after `F-0024` admission; support closure may link `F-0023` report refs, `F-0024` auth evidence refs and `F-0026` release refs read-only | Support code writing runtime, reporting, auth, release, governor, lifecycle, model-serving or perimeter source truth directly | Support rows are derivative incident evidence and procedure truth only. They link canonical owner refs, record redacted notes and classify actions as `owner_routed` or `human_only`; they are not a second reporting, release, auth or governor ledger. |
...
- Operator namespace (`GET /state`, `GET /timeline`, `GET /episodes`, `GET /models`, `GET /reports`, `GET /support/runbooks`, `GET /support/incidents`, `POST /support/incidents`, `PATCH /support/incidents/:id`, `POST /control/tick`) принадлежит `F-0013` как bounded read/control HTTP seam.
- `GET /models` в этом operator namespace ограничен baseline diagnostics из `F-0008` и теперь проецирует bounded richer summary, sourced from `F-0014`; richer registry source state itself remains `F-0014`-owned.
- `POST /control/freeze-development` принадлежит `F-0013`-owned operator namespace and delegates durable freeze semantics to the `F-0016` governor gate only after `F-0024` caller admission and `governor_operator` authorization succeed; missing admission, permission or downstream owner availability fails closed without ad hoc governor writes.
- `POST /control/development-proposals` принадлежит тому же `F-0013`-owned operator namespace and delegates proposal submission to the `F-0016` governor gate only after `F-0024` caller admission and `governor_operator` authorization succeed; missing admission, permission or downstream owner availability fails closed without a parallel proposal-write route.
- Support routes in that namespace belong to `F-0028` procedure/evidence truth only. `GET /support/runbooks` and `GET /support/incidents` are protected reads; `POST /support/incidents` and `PATCH /support/incidents/:id` require `support_operator` admission and may write only support-owned evidence rows while linking `F-0023`, `F-0024` and `F-0026` refs read-only.
...
Эта карта отделяет concept coverage от delivery status. Наличие архитектурного описания или backlog owner-а не означает, что seam уже поставлен; delivery claim появляется только у real dossier со статусом `done`. Если у future seam уже есть реальный dossier, статус показывает текущую dossier maturity (`shaped` / `planned` / `in_progress`), а `candidate` оставлен только для backlog-owned seams без dossier.
...
| Platform substrate, deployment cell and verification baseline | `F-0002`, `F-0006`, `F-0007` | `done` | Phase-0 cell, quality gates, deterministic smoke and the canonical `AI SDK + Hono` runtime/dependency baseline are now aligned on the delivered platform substrate. |
...
| Baseline model router and profile continuity | `F-0008` | `done` | Baseline router invariants are delivered; the shared `model_registry` family now coexists with delivered richer source surfaces from `F-0014`, while specialist organs still remain future seams. |
...
| Homeostat and operational guardrails | `F-0012` | `done` | Homeostat is now delivered end-to-end: committed-tick plus periodic evaluation share one canonical evaluator, durable `homeostat_snapshots` and advisory `homeostat.reaction-request` jobs are live on the PostgreSQL/`pg-boss` substrate, and future reporting/governor/lifecycle metrics remain explicitly source-owned and degrade without hidden proxies. |
| Development governor and policy gates | `F-0016` / `CF-016` | `done` | The governor seam is delivered: freeze control, critical homeostat auto-freeze, idempotent proposal persistence, advisory proposal decisions, workshop promotion-package handoff and bounded execution-outcome evidence are live through owner gates; public `F-0013` high-risk routes now reach those gates only through `F-0024` caller admission/RBAC and still fail closed when downstream owner availability is missing, while downstream execution, thaw semantics, reporting and lifecycle consolidation remain separate future-owned seams. |
...
| Operator API and introspection | `F-0013` | `done` | Operator-facing HTTP API beyond minimal health/ingress is delivered as one bounded Hono route family: `/state`, `/timeline`, `/episodes`, `/models`, `/reports`, `/control/tick`, `/control/freeze-development` and `/control/development-proposals`; protected routes are admitted by `F-0024`, baseline model diagnostics stay sourced from `F-0008`, `/models` projects bounded richer summary from delivered `F-0014`, and durable governor semantics remain owned by `F-0016`. |
| Operator authentication, authorization and RBAC | `F-0024` / `CF-024` | `done` | Public operator caller admission is delivered inside the existing `F-0013` Hono namespace: protected routes require a mounted/static versioned principal file, SHA-256 token-hash verification, route-class RBAC, rate limiting, PostgreSQL auth-decision audit, bounded trusted-ingress evidence, and fail-closed behavior for missing/corrupt auth config or unavailable audit storage. |
| Expanded model ecology and registry health | `F-0014` | `done` | The richer model ecology seam is now delivered on the canonical runtime path: one physical `model_registry` family carries baseline plus richer rows with explicit `service_id` continuity, companion source tables `model_profile_health` and `model_fallback_links` are live, `vllm-deep` / `vllm-pool` degrade into bounded unavailable/degraded source diagnostics without becoming boot-critical, and derived reporting remains separate with `CF-015`. |
| Workshop training/eval/promotion pipeline | `F-0015` | `done` | Workshop delivery is now live on the canonical PostgreSQL/`pg-boss` substrate: durable dataset/training/eval/candidate lifecycle surfaces and bounded promotion-package projection are implemented, queue-family wiring starts in bounded degraded mode, and approval/activation overlays remain future-owned by neighbouring seams. |
...
| Skills procedural layer and package boundary | `F-0022` / `CF-013` | `done` | The skills seam is delivered as repo-owned package truth: `@yaagi/skills` validates and loads canonical `SKILL.md` packages, `apps/core` materializes `seed/skills` into `workspace/skills`, runtime keeps valid-only and active skill lists with workspace-only reload plus owner-only diagnostics, and lifecycle states or a separate registry remain explicitly out of scope. |
| Security/perimeter hardening | `CF-014` | `done` | Mature perimeter hardening is delivered as one fail-closed seam over the canonical runtime: separately reviewable safety kernel, durable perimeter-decision audit, rollback-gate integration, secret-hygiene fail-closed publication guards, bounded restricted-shell/egress regression coverage, and read-only validation of `F-0024` trusted-ingress evidence for admitted `F-0013` high-risk operator paths. |
| Support / operability contract and incident discipline | `F-0028` / `CF-026` | `done` | Supportability is delivered as support-owned runbooks, incident taxonomy, support evidence bundles, support incident storage and protected support routes inside the existing `F-0013` namespace. Support closure links canonical `F-0023`, `F-0024` and `F-0026` refs read-only, redacts operator notes, blocks unsafe critical closure, and does not create a second control plane or foreign owner ledger. |
| Operator-only Telegram conversational egress | `F-0029` / `CF-029` | `in_progress` | Implementation-stage delivery adds explicit egress activation, `telegram.sendMessage` as a bounded executive action, `telegram_egress_messages` owner outbox evidence, fake Bot API send support, operator-only admission and smoke assertions without changing `F-0005` ingress ownership or adding a public bot/webhook path. |
...
- для dossier-backed future seams и candidate seams без dossier указаны owners, так что следующий intake/spec step может опираться на явные cross-cutting contracts — **да**;
