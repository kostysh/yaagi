...
| Workshop source surfaces (`datasets`, `training_runs`, `eval_runs`, candidate-registration and promotion-package manifests) | Workshop seam (`F-0015`) | `F-0014` registry owners, future governor gates, observability and specialist lifecycle seams read workshop evidence or consume bounded packages through canonical gates | Boot/runtime/router/operator helpers writing workshop rows directly, workshop mutating active profile pointers or richer registry rows directly | Workshop owns dataset/training/eval lineage plus bounded candidate/promotion preparation only. Live activation, richer registry writes and specialist lifecycle policy remain with neighbouring owners. |
...
| Workshop training/eval/promotion pipeline | `F-0015` | `proposed` | Workshop datasets, training, eval and bounded promotion-package preparation now have an intaken dossier owner; delivery is still deferred. |
