...
| Operator API and introspection | `F-0013` | `planned` | Operator-facing HTTP API beyond minimal health/ingress is now planned as one bounded Hono route family with explicit delivery slices, DTOs, cursor/idempotency semantics and owner-routed control handoff, while richer model ecology and governor-backed control execution remain explicitly future-owned by `CF-010` and `CF-016`. |
