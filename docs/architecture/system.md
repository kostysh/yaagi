...
- `POST /control/development-proposals` принадлежит тому же `F-0013`-owned operator namespace как reserved future governor path; до delivery `CF-016` он не должен появляться вне этого HTTP boundary и не должен materialize-иться как ad hoc proposal-write route.
...
| Development governor and policy gates | `CF-016` | `shaped` | The first governor dossier now fixes canonical ownership, bounded operator/internal gates and advisory approval boundaries, but no delivered governor seam exists yet. |
