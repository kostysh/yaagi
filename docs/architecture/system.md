...
- `POST /control/freeze-development` принадлежит `F-0013`-owned operator namespace and may delegate durable freeze semantics to the `F-0016` governor gate only after `CF-024` caller admission exists; until then it must remain an explicit unavailable control, not an ad hoc governor write path or stopgap auth surface.
- `POST /control/development-proposals` принадлежит тому же `F-0013`-owned operator namespace and may delegate proposal submission to the `F-0016` governor gate only after `CF-024` caller admission exists; until then it must remain explicit unavailable and may not materialize as an ad hoc proposal-write route.
...
| Development governor and policy gates | `F-0016` / `CF-016` | `done` | The governor seam is delivered: freeze control, critical homeostat auto-freeze, idempotent proposal persistence, advisory proposal decisions, workshop promotion-package handoff and bounded execution-outcome evidence are live through owner gates; the public `F-0013` high-risk routes remain explicit unavailable until `CF-024` caller admission exists, while downstream execution, thaw semantics, reporting and lifecycle consolidation remain separate future-owned seams. |
...
| Operator API and introspection | `F-0013` | `done` | Operator-facing HTTP API beyond minimal health/ingress is delivered as one bounded Hono route family: `/state`, `/timeline`, `/episodes`, `/models`, `/control/tick`, plus reserved high-risk routes `/control/freeze-development` and `/control/development-proposals` that remain explicit unavailable until `CF-024` caller admission exists; baseline model diagnostics stay sourced from `F-0008`, `/models` projects bounded richer summary from delivered `F-0014`, and durable governor semantics remain owned by `F-0016`. |
...
| Security/perimeter hardening | `CF-014` | `done` | Mature perimeter hardening is delivered as one fail-closed seam over the canonical runtime: separately reviewable safety kernel, durable perimeter-decision audit, rollback-gate integration, secret-hygiene fail-closed publication guards, bounded restricted-shell/egress regression coverage, and explicit unavailable public high-risk operator routes until `CF-024` caller admission exists. |
