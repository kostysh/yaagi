...
- `GET /models` в этом operator namespace ограничен baseline diagnostics из `F-0008` и later may project a bounded richer summary sourced from `F-0014`; richer registry source state itself remains `F-0014`-owned.
...
| Expanded model ecology and registry health | `F-0014` | `planned` | One canonical dossier now owns the future-expanded organ registry source state, richer source diagnostics and explicit fallback/quarantine metadata beyond the delivered baseline router slice, while derived reporting remains separate and delivery is split into implementation-ready slices. |
