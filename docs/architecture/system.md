...
| Development/governance proposal surfaces (`development_ledger`, governor-side proposal records, proposal decisions, freeze state) | Future governor seam (`CF-016`) | Runtime, recovery, workshop, human override and later code/body evolution flows may submit evidence or proposal requests through governor-owned gates | Boot/runtime/router/subject-state seams writing arbitrary proposal rows directly; code/body evolution writing governor rows outside governor-owned gates | Proposal, decision, freeze and ledger writes must flow through governor policy gates and preserve evidence plus rollback links. Workshop may submit candidate evidence or promotion packages, and `CF-012` may later consume approval/rollback evidence for execution, but neither seam owns the governor source surfaces themselves. |
...
| Development governor and policy gates | `F-0016` / `CF-016` | `in_progress` | The governor seam is being delivered in packages: freeze control, critical homeostat auto-freeze, operator proposal submission, idempotent proposal persistence and advisory proposal decisions are live through owner gates; workshop/evidence handoff and final drift closure remain in progress. |
