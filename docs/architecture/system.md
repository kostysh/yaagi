...
- `POST /control/freeze-development` принадлежит `F-0013`-owned operator namespace and delegates all durable freeze semantics to the `F-0016` governor gate once that seam is delivered; before that delivery it must remain explicit unavailable control, not an ad hoc governor write path.
- `POST /control/development-proposals` принадлежит тому же `F-0013`-owned operator namespace and delegates proposal submission to the `F-0016` governor gate once proposal lifecycle is delivered; before that delivery it must not appear outside this HTTP boundary or materialize as an ad hoc proposal-write route.
...
| Development governor and policy gates | `F-0016` / `CF-016` | `done` | The governor seam is delivered: freeze control, critical homeostat auto-freeze, operator proposal submission, idempotent proposal persistence, advisory proposal decisions, workshop promotion-package handoff and bounded execution-outcome evidence are live through owner gates; downstream execution, thaw semantics, reporting and lifecycle consolidation remain separate future-owned seams. |
