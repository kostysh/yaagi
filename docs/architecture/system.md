...
| Expanded model ecology and registry health | `F-0014` | `shaped` | One canonical dossier now owns the future-expanded organ registry source state, richer source diagnostics and explicit fallback/quarantine metadata beyond the delivered baseline router slice, while derived reporting remains separate. |
