...
| Homeostat and operational guardrails | `F-0012` | `planned` | Homeostat now has an implementation plan split across contract/persistence, committed-state evaluation, advisory reaction routing and dual-cadence runtime closure, while canonical source surfaces for reporting, governor and lifecycle metrics remain explicitly future-owned. |
