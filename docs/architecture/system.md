...
| Development/governance proposal surfaces (`development_ledger`, governor-approved model/code/policy proposals) | Future governor/code-evolution seams (`CF-016`, `CF-012`) | Runtime, recovery, workshop and human override may submit evidence or incidents through governor-owned gates | Boot/runtime/router/subject-state seams writing arbitrary proposal rows directly | Proposal and ledger writes must flow through policy gates and preserve evidence plus rollback links. Workshop may submit candidate evidence or promotion packages, but it does not own final governance approval. |
