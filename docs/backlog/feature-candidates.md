...
| CF-004 | Буфер восприятия и сенсорные адаптеры | восприятие | candidate | CF-001, CF-002 | Нормализация входящего потока, буфер восприятия и первый набор адаптеров (`http`, `file`, `scheduler`, `resource`, `system`, `telegram`) образуют отдельную входную подсистему (`4.2.3`, `8.1`, `9.2`). | - |
