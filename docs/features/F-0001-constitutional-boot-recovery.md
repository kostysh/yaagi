...
- **AC-F0001-02:** Boot sequence выполняет health checks PostgreSQL и обязательных model services, вычисляет startup mode по policy-таблице (`normal`, `degraded`, `recovery`) и записывает boot event в timeline с полями `mode`, `dependency_results`, `schema_version` и `snapshot_id|null`.
...
| AC-F0001-02 | `apps/core/test/runtime/boot.integration.test.ts` → `test("AC-F0001-02 emits boot event with selected startup mode and dependency results")` | done |
