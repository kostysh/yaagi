...
const requireUrl = (value: string, label: string): string => {
  ...
  if (url.protocol !== "http:" && url.protocol !== "https:" && url.protocol !== "postgres:" && url.protocol !== "postgresql:") {
...
const parsePositiveInteger = (value: string | undefined, label: string, fallback: number): number => {
...
export function loadCoreRuntimeConfig(env: NodeJS.ProcessEnv = process.env): CoreRuntimeConfig {
  ...
  return {
    ...
    workspaceBodyPath: resolvePath(
      cwd,
      parsedEnv.YAAGI_WORKSPACE_BODY_PATH,
      "workspace/body",
    ),
