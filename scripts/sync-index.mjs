...
const featureRow = (dossier) => {
  const dependsOn =
    Array.isArray(dossier.depends_on) && dossier.depends_on.length
      ...
      : "—";
  const impacts =
    Array.isArray(dossier.impacts) && dossier.impacts.length
      ? dossier.impacts.join(",")
      : "—";
...
const main = async () => {
  ...
  for (const fileName of dossierFiles) {
    ...
    if (!frontmatter) {
      console.warn(`[sync-index] WARN: missing or invalid frontmatter: ${path.join(dossiersDir, fileName)}`);
      continue;
    }

    const relPath = path
      .relative(path.dirname(absIndexFile), absFile)
      .split(path.sep)
      .join("/");
