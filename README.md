...
- Telegram egress is off by default; enabling `YAAGI_TELEGRAM_EGRESS_ENABLED=true` requires `YAAGI_TELEGRAM_OPERATOR_CHAT_ID`, a bot token and the same operator chat id in `YAAGI_TELEGRAM_ALLOWED_CHAT_IDS`
...
- support and operability routes live inside the same protected Operator API namespace: `GET /support/runbooks`, `GET /support/incidents`, `POST /support/incidents`, and `PATCH /support/incidents/:id`; support writes require the `support_operator` role and may write only support-owned incident/evidence rows while linking reporting, release and auth facts by canonical refs
